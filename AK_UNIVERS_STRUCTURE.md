...
- **⭐ Simplicité « Apple-grade » = NON-NÉGOCIABLE** *(2026-06-13, après test live par Adama)* : l'app est trop chargée (accueil ~12 blocs, menu ~20 items). Cible iPhone : **FRONT ultra-simple, complexité CACHÉE (pas supprimée)**. Règles : **menu 5 entrées max** + écran « Tout AK » (App Library) + recherche · **accueil = 1 bloc principal** (« ce que tu dois faire maintenant ») + minimum · progressive disclosure (rien tant que pas besoin) · de l'air. **Test** : l'utilisateur sait quoi faire en 3 s. → ne JAMAIS re-empiler.
- **⭐ Positionnement : vendre le RÉSULTAT, PAS « l'IA »** *(2026-06-13)* : beaucoup d'utilisateurs se méfient de l'IA → **ne pas crier « IA ».** L'IA = **moteur caché, discret** (comme Apple/Amazon ne vendent pas « du machine learning »). Le landing/le produit vend la **transformation** (« ta vie pro gérée, rien d'oublié, tu avances »), pour **tous les métiers**. **La magie se RESSENT (l'expérience, la démo populée), elle ne s'annonce pas.** Landing = premium/glacial, bénéfice d'abord, 1 action, preuve sociale + tarifs.
...
- **Audit du cœur DEVIS → FACTURE → PAIEMENT (2026-06-11)** : la boucle est **structurellement complète et persiste bien** (schéma sain, RLS OK, statuts/retard/relances câblés, PDF généré, build vert + 29 tests). MAIS elle **n'a jamais tourné sur du réel** (0 client / 0 devis / 0 facture en base). 3 faiblesses + plan de réparation (ordre de levier) :
  ...
  5. ✅ **AUDIT FAIT (2026-06-11) — base Supabase** (lecture seule via MCP, écrit dans `docs/DB_SCHEMA.md`). 78 tables · 100% RLS · 12 avec données.
     ...
     - 🔴 **Dettes à traiter PAR LOTS** (jamais un gros refactor) :
       - **Lot 1 (sûr) — intégrité + sécurité** : ✅ **FAIT (2026-06-11)**. Vague 0 sécurité (search_path advisor 1→0) + Vague 1 FK manquantes (6 FK posées, ON DELETE par table : CASCADE données privées (employes/planning), SET NULL communautaire (forum, anonymisé) + liens métier (client/facture)). Build vert, migrations idempotentes, poussées.
       - **Lot 2 / Vague 2 — nettoyage** : §4.7 vérif d'usage frontend **FAITE** → seulement **4 tables vraiment mortes** (0 réf code + 0 ligne) à DROP : `progression`, `modules_formation`, `abonnements`, `mentions_legales` (contenu légal servi par `lib/conformite.js`, pas la table). Les « doublons » suspectés (formations_perso/suivies, planning_contenu/contenus) sont **VIVANTS** → fusion = refactor de code distinct, **plus tard**. **La base est bien moins gonflée que craint** (la cible « ~35 » était surestimée). ← EN COURS
       ...
       - **Lot/Vague 3 — RGPD & suppression de compte** : ✅ **FAIT (2026-06-11)**. Bug critique corrigé (suppression de compte était cassée pour tout user réel) + conforme RGPD, prouvé par cycle réel. Règle : CASCADE des 7 tables personnelles, **anonymise + conserve** les 4 financières (factures/devis/contrats/paiements) pour l'obligation légale de conservation (~10 ans) ; forum anonymisé (« Utilisateur supprimé »). Edge function `delete-account` live.
         - *Résiduel* : `timestamptz` ✅ **FAIT (2026-06-11)** — 10 colonnes converties, fuseaux FR/Afrique corrects (sert l'interaction diaspora aux bonnes heures). **Mission base entièrement bouclée (V0→V3b).** Reste parké : uniformisation RLS = **à faire DANS la mission Monétisation** (dépend des paliers équipe) ; 2 fusions (formations_perso/suivies, planning_contenu/contenus) = refactor de code plus tard ; à confirmer avec expert-comptable/juriste avant lancement + ajouter « exporter mes données » avant suppression.
