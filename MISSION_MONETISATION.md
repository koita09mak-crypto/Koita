...
>
> **Plan par vagues** : **M0** ✅ **FAIT (2026-06-12)** — compteur blindé (réservation atomique « réserver→appeler→réconcilier », prouvé en base : 3ᵉ résa au-delà du plafond refusée, réconciliation OK, build vert). Marge garantie. → **M1** plafonner factures (**décision : 3/mois sur le gratuit, miroir devis**) → **M2** routage Haiku → **M3** prouver le flux paiement (webhook simulé) → **M4** cleanup + cohérence.
